...
class Quiz
  ...
  def create_question
    loop do
      ...
      break Question.new(statement) if statement
  ...
  def list_questions
    @questions.each_with_index do |question, index|
      puts "Question #{index + 1}: #{question.statement}"
      question.options.each_with_index do |option, opt_index|
        puts "#{opt_index + 1}. #{option.text}"
      end
    end
  end
  ...
  def unlock!(require_confirmation = true)
    common_input_for_publish_lock('un-lock') if require_confirmation
  ...
  def publish!(require_confirmation = true)
    common_input_for_publish_lock('publish') if require_confirmation
  ...
  def unpublish!(require_confirmation = true)
    common_input_for_publish_lock('un-publish') if require_confirmation

    @published = false
    puts 'Quiz Un-published successfully!'
  end

  def attempt(student_email)
    student_score = 0
    list_questions
    @questions.each_with_index do |question, index|
      print 'Your answer: '
    ...
    @attempts += 1
    @attempt_log[student_email] ||= []
    attempt_record = { date: Time.now, score: student_score, attempts: @attempts }
    @attempt_log[student_email] << attempt_record
    student_score
  end
  ...
  def view_attempts(student_email)
    ...
    attempts.each do |attempt|
      puts "Attempt Date: #{attempt[:date]}, Score: #{attempt[:score]}, Attempts: #{attempt[:attempts]}"
    end
    puts "Total Attempts for #{student_email} are #{@attempts}."
  end
